...
function RecordsList_Records({userData, branch, date, search}){
    ...
    useEffect(() => {
        ...
        axios.post(localStorage.getItem('urlLink')+'RecordsSearch_getRecords.php', data)
            .then(response => {
...
{data.length > 0? (
    ...
    {openInfo && <TransactionInfo closeModal={setInfo} branch={branch} userData={userData} transactionID={transactionIDData}/> }
    </div>
    )
}
